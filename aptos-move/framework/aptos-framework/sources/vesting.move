...
module aptos_framework::vesting {
    ...
    #[test(aptos_framework = @0x1, admin = @0x123, shareholder = @0x234, operator = @0x345, beneficiary = @0x456)]
    public entry fun test_set_beneficiary_for_operator(
        ...
        setup(aptos_framework, &vector[admin_address, shareholder_address, operator_address, beneficiary_address]);
        let contract_address = setup_vesting_contract(
            admin, &vector[shareholder_address], &vector[GRANT_AMOUNT], admin_address, 0);
        assert!(operator_commission_percentage(contract_address) == 0, 0);
        let stake_pool_address = stake_pool_address(contract_address);
        // 10% commission will be paid to the operator.
        update_operator(admin, contract_address, operator_address, 10);
        assert!(stake::get_beneficiary_for_operator(stake_pool_address) == operator_address, 0);
        set_beneficiary_for_operator(operator, contract_address, beneficiary_address);
        assert!(stake::get_beneficiary_for_operator(stake_pool_address) == beneficiary_address, 0);
        ...
        assert!(coin::balance<AptosCoin>(operator_address) == 0, 1);
